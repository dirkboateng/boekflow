...
export default async function PublicBusinessPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-cream">
      <header className="px-6 py-16 md:py-24 text-cream" style={{ backgroundColor: brandColor }}>
        <div className="max-w-3xl mx-auto">
          <div className="text-xs uppercase tracking-wider text-cream/60 mb-3" style={{ letterSpacing: "0.12em" }}>
            {business.category}
          </div>
          <h1 className="font-display font-semibold text-cream mb-4" style={{ fontSize: "clamp(40px, 7vw, 80px)", letterSpacing: "-2px", lineHeight: "1" }}>
      ...
      <main className="px-6 py-16 max-w-3xl mx-auto">
          ...
          {!services || services.length === 0 ? (
            ...
            <div className="space-y-3">
              {services.map((service) => (
                <div key={service.id} className="bg-paper border border-line rounded-2xl p-6 flex items-center justify-between hover:border-ink transition">
                  <div className="flex-1">
                    <h3 className="font-display font-semibold text-ink mb-1" style={{ fontSize: "20px", letterSpacing: "-0.6px", lineHeight: "1.2" }}>
                      {service.name}
                    </h3>
                    {service.description && (
                    ...
                    <div className="text-xs text-slate">{service.duration_minutes} minuten</div>
                  </div>
                  <div className="ml-6 text-right">
                    <div className="font-display font-semibold text-ink mb-2" style={{ fontSize: "22px", letterSpacing: "-0.8px", lineHeight: "1" }}>
                      €{(service.price_cents / 100).toFixed(2).replace(".", ",")}
                    </div>
                    <Link href={`/b/${slug}/boek/${service.id}`} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-[10px] text-cream font-medium text-sm hover:opacity-90 transition" style={{ backgroundColor: brandColor, letterSpacing: "-0.2px" }}>
                      Boeken
        ...
        <section className="bg-paper border border-line rounded-2xl p-8">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <h3 className="font-display font-semibold text-ink mb-1" style={{ fontSize: "18px", letterSpacing: "-0.5px", lineHeight: "1.2" }}>
            ...
            {whatsappNumber && (
              <a href={`https://wa.me/${whatsappNumber}`} target="_blank" rel="noopener noreferrer" className="px-4 py-2.5 rounded-[10px] bg-ink text-cream font-medium text-sm hover:bg-ink-soft transition" style={{ letterSpacing: "-0.2px" }}>
                Open WhatsApp
              </a>
            )}
          </div>
